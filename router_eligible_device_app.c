...
#define APP_TEAM5_URI_PATH                       "/team5"
#else
#define APP_TEAM5_ROUTER_URI_PATH                "/team5"
...
#if(defined(LEADER)&&LEADER)
static void APP_Coapteam5Cb(coapSessionStatus_t sessionStatus, void *pData, coapSession_t *pSession, uint32_t dataLen);

const coapUriPath_t gAPP_TEAM5_URI_PATH = {SizeOfString(APP_TEAM5_URI_PATH),(uint8_t *)APP_TEAM5_URI_PATH};

#else
static void APP_Coapteam5_routerCb(coapSessionStatus_t sessionStatus, void *pData, coapSession_t *pSession, uint32_t dataLen);

const coapUriPath_t gAPP_TEAM5_ROUTER_URI_PATH = {SizeOfString(APP_TEAM5_ROUTER_URI_PATH),(uint8_t *)APP_TEAM5_ROUTER_URI_PATH};
...
static void myTaskTimerCallback(void *param)
{
	 uint8_t *pMySessionPayload=&counter;
	  ...
	  COAP_AddOptionToList(pMySession,COAP_URI_PATH_OPTION, APP_TEAM5_ROUTER_URI_PATH,SizeOfString(APP_TEAM5_ROUTER_URI_PATH));
	  FLib_MemCpy(&pMySession->remoteAddrStorage.ss_addr, &gLeaderAddress, sizeof(ipAddr_t));
...

{
  ...
  pMySession = COAP_OpenSession(mAppCoapInstId);
  volatile uint32_t addr=pSession->remoteAddrStorage.ss_addr;
  COAP_AddOptionToList(pMySession,COAP_URI_PATH_OPTION, APP_TEAM5_URI_PATH,SizeOfString(APP_TEAM5_URI_PATH));
...
#else
static void APP_Coapteam5_routerCb
(
coapSessionStatus_t sessionStatus,
void *pData,
coapSession_t *pSession,
...
)
{
	...
	PORT_SetPinConfig(PORTC, 2, &portc2_pin38_config);  /* PORTC2 (pin 38) is configured as I2C1_SCL */
	...
	PORT_SetPinConfig(PORTC, 3, &portc3_pin39_config);  /* PORTC3 (pin 39) is configured as I2C1_SDA */
	...
	I2C_MasterTransferCreateHandle(I2C1, &g_MasterHandle, NULL, NULL);

	fxosHandle.xfer.slaveAddress=0x1FU;
...
)
{
    ...
    switch(pEventParams->code)
    {
        ...
        case gThrEv_GeneralInd_Connected_c:
            ...
            THR_GetLeaderRLOCIpAddr(mThrInstanceId,&gLeaderAddress);
...
)
{
    coapRegCbParams_t cbParams[] =  {{APP_CoapLedCb,  (coapUriPath_t *)&gAPP_LED_URI_PATH},
									...
									#if(defined(ROUTER)&&ROUTER)
									 {APP_Coapteam5_routerCb, (coapUriPath_t*)&gAPP_TEAM5_ROUTER_URI_PATH},
